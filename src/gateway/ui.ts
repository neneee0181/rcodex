...
<div class="layout">
  ...
  <div class="sb" id="sb">
    <div class="sb-hdr">
      ...
      <button class="sb-x" onclick="toggleSb()">Ã—</button>
    </div>
  ...
  <div class="ws" id="ws">
    ...
    <div class="hint">Scroll to zoom Â· Drag to pan Â· Drag ??to connect</div>
  </div>
  ...
  if(sbScreen==='home'){
    ...
    body.innerHTML=\`
      <div style="padding:8px 0">
        ...
        <div class="nav-item" style="opacity:.4;pointer-events:none">
          <div class="nav-ic" style="background:rgba(96,96,128,.1)">?“Š</div>
          <div class="nav-info">
    ...
    body.innerHTML=\`
      <div class="auth-form">
        ...
        <div style="font-size:10px;color:var(--mu);padding-top:8px">Waiting for approval Â· expires in ~\${mins}m</div>
      </div>\`;
  }
}
    ...
    return \`<div class="acc-item" style="flex-direction:column;align-items:stretch;gap:5px">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div class="acc-info">
          ...
          <div class="acc-sub">\${methodLabel[a.method]||a.method}\${activeSlots?' Â· <span style="color:var(--gr)">??/span> '+activeSlots+' active':canvasCount?' Â· '+canvasCount+' on canvas':''}</div>
          \${sub?\`<div class="acc-sub" style="opacity:.6;margin-top:1px;font-size:9px">\${sub}</div>\`:''}
        </div>
        <button class="del-btn" onclick="deleteAccount('\${a.id}')" title="Delete">Ã—</button>
      </div>
...
function buildMonitorNode(){
  ...
  <div class="nh" style="border-radius:13px 13px 0 0;cursor:grab">
    ...
    <button class="nd-rm" onclick="toggleMonitor(monitorTab)" title="Close">Ã—</button>
  </div>
  ...
  try{
    ...
    const rows=d.requests.map((r,i)=>{
      ...
      if(hasDetail){
        ...
        if(r.webFetches?.length) parts.push(\`<div class="req-detail-section"><div class="req-detail-label">?Œ Web fetched</div><div class="req-detail-val">\${r.webFetches.map(u=>\`<code>\${escHtml(u)}</code>\`).join('<br>')}</div></div>\`);
...
function renderUsage(){
  ...
  const note=\`<div style="padding:6px 10px;font-size:9px;color:var(--mu);border-top:1px solid var(--b1);margin-top:8px">Tokens: resets at midnight Â· Quota: manual refresh</div>\`;
...
function accountSubtext(acc){
  ...
  if(acc.method==='apikey'&&acc.apiKey){
    const k=acc.apiKey;
    return '?”‘ '+k.slice(0,10)+'Â·Â·Â·'+k.slice(-4);
  }
  if(acc.method==='oauth-unofficial')return '?ª Session Token';
...
function buildAccNode(slotId){
  ...
  <div class="nh">
    ...
    <button class="nd-rm" onclick="removeFromCanvas('\${slotId}')" title="Remove from canvas">Ã—</button>
  </div>
...
function buildOutNode(){
  ...
  const subtitle=multi?'Priority order Â· fallback chain':'Codex uses these models';
  const body=allSlots.length
    ?allSlots.map(({acc,slot},i)=>{
        ...
        return \`<div class="oi">
          ...
          \${orderBtns}
          <button class="oi-x" onclick="removeOut('\${acc.id}','\${slot.slotId}')">Ã—</button>
        </div>\`;
      }).join('')
