...
function parseCodexToolOutput(output: unknown): string {
  ...
  if (idx !== -1) {
    ...
    if (!actual && exitCode === 0) return "(ë¹?ì¶œë ¥ ??ê²°ê³¼ ?†ìŒ)";
    if (!actual) return `(?¤ë¥˜: exit code ${exitCode} ???´ë‹¹ ê²½ë¡œ???Œì¼ ?†ìŒ)`;
    return actual;
  }
  return raw.trim();
}
...
function fallbackToolResultText(output: unknown): string {
  ...
  if (!text || text === "(ì¶œë ¥ ?†ìŒ)") return "???¤í–‰ ?„ë£Œ. ì¶œë ¥ ?†ìŒ.";
  return `???¤í–‰ ê²°ê³¼:\n${text}`;
}
...
async function* streamSingleProvider(
  ...
  if (account.provider === "ollama") {
    ...
    while (true) {
      ...
      for (const call of webFetchCalls) {
        ...
        try {
          ...
          const header = [title && `?œëª©: ${title}`, metaDesc && `?¤ëª…: ${metaDesc}`].filter(Boolean).join("\n");
          const body = text
          ...
          content = `ê°€?¸ì˜¤ê¸??¤íŒ¨: ${e instanceof Error ? e.message : String(e)}`;
        }
    ...
    if (capturedOllamaTools.length === 0 && capturedOllamaText.value.trim() === "" && currentToolOutputs.length > 0) {
      ...
      if (!parsed || parsed.includes("ê²°ê³¼ ?†ìŒ") || parsed.includes("?´ë‹¹ ?Œì¼/??ª©??ì¡´ìž¬?˜ì? ?ŠìŠµ?ˆë‹¤")) {
        fallbackText = parsed || "ëª…ë ¹???¤í–‰?ˆì?ë§?ê²°ê³¼ê°€ ?†ìŠµ?ˆë‹¤.";
      } else {
